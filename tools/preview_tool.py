...
preview_tools = [
    {
        ...
        "function": {
            "name": "preview_scan",
            "description": (
                "Scan file atau direktori untuk mendeteksi error sintaks, "
                "import rusak, atau masalah umum sebelum preview. "
                "Dukung: Python (.py), JavaScript (.js), TypeScript (.ts/.tsx), "
            ...
            "parameters": {
                ...
                "properties": {
                    "path": {
                        "type": "string",
                        "description": "Path file atau direktori yang mau di-scan.",
                    },
                    "recursive": {
                        "type": "boolean",
                        "description": "Scan subdirektori juga (default: false).",
                    },
                },
                "required": ["path"],
            },
    ...
    },
    {
        ...
        "function": {
            "name": "preview_web",
            "description": (
                "Launch web preview untuk file HTML / direktori web project. "
                "Serve via HTTP lokal dan buka di browser default. "
                "Kalau file Python/JS, jalankan dulu lalu preview output-nya."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "path": {
                        "type": "string",
                        "description": "Path file HTML atau direktori yang mau di-preview.",
                    },
                    "port": {
                        "type": "integer",
                        "description": "Port HTTP server (default: auto-detect free port).",
                    },
                    "auto_open": {
                        "type": "boolean",
                        "description": "Buka browser otomatis (default: true).",
                    },
                },
                "required": ["path"],
            },
...
def _scan_json(path: str) -> list[str]:
    import json
    errors = []
    try:
...
def _scan_html(path: str) -> list[str]:
    ...
    try:
        ...
        if re.search(r'<script\s[^>]*src=[^>]*>\s*</script>', content, re.IGNORECASE):
            pass  # normal
...
def _scan_css(path: str) -> list[str]:
    errors = []
    try:
        with open(path, encoding="utf-8", errors="replace") as f:
            content = f.read()
...
def scan_path(path: str, recursive: bool = False) -> dict:
    """Scan file/direktori. Return dict { filename: [errors] }."""
    p = Path(path)
    results = {}
    ...
    if p.is_file():
        ...
        if scanner:
            errs = scanner(str(p))
            results[str(p)] = errs if errs else []
        else:
            results[str(p)] = [f"INFO: Tipe file '{ext}' tidak di-scan otomatis."]
        return results
    ...
    if p.is_dir():
        ...
        for child in p.glob(pattern):
            if child.is_file():
                ...
                scanner = SCANNERS.get(ext)
                if scanner:
                    errs = scanner(str(child))
                    results[str(child)] = errs if errs else []
        return results
...
def _serve_directory(directory: str, port: int):
...
async def handle_preview_tool(name: str, args: dict) -> str:
    path       = args.get("path", ".")
    ...
    if name == "preview_scan":
        ...
        if p.is_file() and p.suffix.lower() in (".html", ".htm"):
            serve_dir  = str(p.parent)
            entry_file = p.name
        elif p.is_dir():
            serve_dir  = str(p)

            candidates = ["index.html", "index.htm", "public/index.html", "dist/index.html"]
            entry_file = None
            for c in candidates:
                if (p / c).exists():
                    entry_file = c
            ...
            if not entry_file:
                ...
                if htmls:
                    entry_file = htmls[0].name
                else:
                    return "✖ Tidak ada file HTML ditemukan di direktori ini. Preview hanya support HTML untuk sekarang."
        else:
            return f"✖ File '{path}' bukan HTML dan bukan direktori web. Preview hanya support HTML."
        ...
        if auto_open:
            try:
                webbrowser.open(url)
                open_msg = "  ↳ Browser dibuka otomatis."
            except Exception:
                open_msg = "  ↳ Gagal buka browser otomatis. Buka manual ya."
        else:
            open_msg = "  ↳ Auto-open dimatikan."
